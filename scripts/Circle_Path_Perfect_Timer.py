...
class Path():
    ...
    def ref_update(self):
        self.ref_msg.pose.position.x = self.x_o+self.mag*math.sin(self.speed*(self.t-self.t_o))
        self.ref_msg.pose.position.y = self.y_o+self.mag*math.cos(self.speed*(self.t-self.t_o))
    ...
    def traj_update(self):
        ...
        for i in range(0,self.future_length):
            velocities = Twist()
            accelerations = Twist()
            x = self.x_o+self.mag*math.sin(self.speed*((self.t-self.t_o)+i*self.dt))
            y = self.y_o+self.mag*math.cos(self.speed*((self.t-self.t_o)+i*self.dt))
            z = self.z_o
            velocities.linear.x = self.speed*self.mag*math.cos(self.speed*((self.t-self.t_o)+i*self.dt))
            velocities.linear.y = self.speed*-self.mag*math.sin(self.speed*((self.t-self.t_o)+i*self.dt))
            velocities.linear.z = 0.0
            accelerations.linear.x = self.speed*self.speed*-self.mag*math.sin(self.speed*((self.t-self.t_o)+i*self.dt))
            accelerations.linear.y = self.speed*self.speed*-self.mag*math.cos(self.speed*((self.t-self.t_o)+i*self.dt))
